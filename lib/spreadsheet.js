...
Spreadsheet.prototype.request = function(opts, callback) {
  ...
  opts.headers = _.extend({}, this.authHeaders, opts.extraHeaders);
  ...
  request(opts, function(err, response, body) {
    ...
    var result;
    try {
      result = xmlutil.toJson(body, {object: true, sanitize: opts.sanitize || false, trim: false, reversible: opts.reversible || false});
    } catch (e) {
...
Spreadsheet.prototype.getSheetId = function(type, callback) {
  ...
  this.request({
    ...
    var m = null;
    if (!_this[id]) {
      if (_this.opts.createWorksheet) {
        _this.addWorksheet(name, _this.opts.createWorksheetRows || 10, _this.opts.createWorksheetCols || 50, 
          function(err, result) {
            if (err) {
              return callback(err);
            }
            _this.log('Worksheet ' + name + ' was created.');
            _this.switchWorksheet(result);
            return callback(null);
          });
      } else {
...
Spreadsheet.prototype.baseUrl = function() {
  var url = this.protocol + '://spreadsheets.google.com/feeds/cells/' + this.spreadsheetId + '/' + this.worksheetId + '/private/full';
  return url;
};

Spreadsheet.prototype.worksheetUrl = function() {
  return this.protocol + '://spreadsheets.google.com/feeds/worksheets/' + this.spreadsheetId + '/private/full';
};

Spreadsheet.prototype.rowsUrl = function() {
  return this.protocol + '://spreadsheets.google.com/feeds/list/' + this.spreadsheetId + '/' + this.worksheetId + '/private/full';
};

Spreadsheet.prototype.switchWorksheet = function(worksheetId) {
  this.worksheetId = worksheetId;
  this.setTemplates();
  this.log("Switched worksheet to " + worksheetId);
};

Spreadsheet.prototype.addWorksheet = function(title, rows, cols, callback) {
  var _this = this;
  var entry = '<entry xmlns="http://www.w3.org/2005/Atom" xmlns:gs="http://schemas.google.com/spreadsheets/2006">' +
    ...
    '<gs:colCount>' + cols + '</gs:colCount></entry>';
  _this.request({
      method: 'POST',
      url: _this.worksheetUrl(),
      body: entry,
    }, function(err, result) {
      if (err) {
        return callback(err);
      ...
      if (result && result.entry && result.entry.id) {
        var parts = result.entry.id.split('/');
        callback(null, parts[parts.length-1]);
      } else {
        return callback("Could not read worksheet ID from response");
      }
    });
};

Spreadsheet.prototype.getRows = function(query, callback) {
  var _this = this;
  var q = "?sq=";
  var first = true;
  if (typeof query === "object") {
    for (var k in query) {
      q += encodeURIComponent((first ? '' : ' and ') + k + '="' + query[k] + '"');
      first = false;
    }
  } else {
    q += query || '';
  ...
  this.request({
    ...
    var info = {
      ...
      totalRows: result.feed['openSearch:totalResults']['$t']
    };
    _.each(entries, function(entry) {
      var row = {
        etag: entry['gd:etag'],
        raw: entry,
        editLink: entry.link.filter(function(l) {return l.rel === 'edit';})[0].href.replace(/^https/, _this.protocol), 
        data: {}
      };
      for (var key in entry) {
        if (key.indexOf('gsx:') === 0) {
          row.data[key.substr(4)] = entry[key];
        }
      }
      rows.push(row);
    });
...
Spreadsheet.prototype.addRow = function(data, callback) {
  var _this = this;
  var entry = '<entry xmlns="http://www.w3.org/2005/Atom" xmlns:gsx="http://schemas.google.com/spreadsheets/2006/extended">';
  var keys = [];
  for (var key in data) {
    keys.push(key);
    var encodedKey = encodeURIComponent(key);
    entry += '<gsx:' + encodedKey + '>' + data[key].toString() + '</gsx:' + encodedKey + '>';
  }
  entry += '</entry>';
  _this.log("Adding row", entry);
  var retry = true;
  var sendRequest = function() {
    _this.request({
      ...
      if (err) {
        if (retry) {
          retry = false;
          // try to add a header line
          keys.forEach(function(key, idx) {
            _this.log("Adding " + key + " at 1," + (idx+1));
            _this.addVal(key, 1, idx + 1);
          });
          return _this.send(function(err) {
            if (err) {
              return callback(err);
            }
            return sendRequest();
...
Spreadsheet.prototype.updateRow = function(row, callback) {
  var _this = this;
  delete row.raw['gd:etag'];
  row.raw.xmlns="http://www.w3.org/2005/Atom";
  row.raw["xmlns:gsx"]="http://schemas.google.com/spreadsheets/2006/extended";

  var xmlData = xmlutil.toXml({entry: row.raw});
  _this.request({
        method: 'PUT',
        url: row.editLink,
        body: xmlData,
